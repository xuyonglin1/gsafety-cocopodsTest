...
public typealias userImage = ((_ url: String) -> ())

class WMCameraManger: NSObject, AVCapturePhotoCaptureDelegate {
    ...
    private var onUserImage: userImage?
...
//        let devicesVideo = AVCaptureDevice.devices(for: .video)
    ...
    // 拍照调用方法
    func pickImage(complete: @escaping userImage) {
...
//            print(UIImage(data: dataImage)?.size as Any)
            let originImage = UIImage.init(data: dataImage)
    ...
    func getCameraDevice(_ position: AVCaptureDevice.Position) -> AVCaptureDevice? {
        let cameras = AVCaptureDevice.default(.builtInWideAngleCamera, for: AVMediaType.video, position: position)
        return cameras
    }
